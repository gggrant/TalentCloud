...
(function ($) {
  ...
  var $root = $('html, body'); // Add has attribute Function
  ...
  $(document).ready(function () {
    ...
    function openModal(trigger) {
      ...
      $("body").css("overflow", "hidden"); // Tab Items
      ...
      $(".modal-delete-trigger").one("click", function (e) {
        ...
        if ($(object).attr('data-item-saved')) {
          ...
          axios.delete(deleteUrl).then(function (response) {
            ...
            $(modal).removeClass("working"); //TODO: deal with error better, localized text at least

            window.alert("Something went wrong deleting item!"); // Allow for retrying
    ...
    function escapeModalHandler() {
      $(document).on("keyup", function (e) {
        if (e.key === 'Escape' || e.key === 'Esc' || e.keyCode === 27) {
          $(".modal-overlay").removeClass("active");
          $(".modal").removeClass("active");
          $("body").css("overflow", "visible"); // FF and compatible
    ...
    function requiredFields() {
      $("input:required, textarea:required").each(function (e) {
        $(this).parent().addClass("required"); //Find labels that haven't had the asterisk added yet
    ...
    labelHandlers(); // AJAX Form Handlers ==========================================
    ...
    function ajaxOnSave(object) {
      return function onSave(response) {
        ...
        $(object).find('button[type=submit]').removeClass('working'); //$(object).find('.accordion-trigger').focus();
      };
    ...
    function submitItem(object) {
      ...
      var formData = $(form).serialize(); //Add working class to submit button

      $(object).find('button[type=submit]').removeClass('saved');
      $(object).find('button[type=submit]').addClass('working');
      var requestPromise; //If object already exists on server, update it
    ...
    function setItemSaved(object, response) {
      ...
      $(object).find('.reveal-on-save').removeClass('hidden'); //$(object).removeClass('active');
    ...
    $('a.confirm-unsaved-data').click(confirmLinkIfUnsavedData); // Individualizing repeater name and id attributes======================
    ...
    function appendToAttributes(parent, attribute, suffix) {
      ...
      var selector = "*[" + attribute + "]"; //If conditions is set, only modify attributes that also
    ...
    function individualizeFormIdsAndNames(template, wrapper) {
      // Get New ID
      var newId = getNextItemId(wrapper); //Set date-item-id, used to track which newId's are taken
    ...
    function cloneRepeatingElement(trigger, parentSelector, listWrapperSelector, nullSelector, templateSelector) {
      var prepend = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : false;
      var parent = $(trigger).parents(parentSelector); // Get List Wrapper

      var wrapper = parent.find(listWrapperSelector); // Set Null to Hidden

      parent.find(nullSelector).removeClass("active"); // Get Template

      var template = parent.find(templateSelector).clone(); // Remove Template Class
    ...
    function addRepeater(trigger) {
      ...
      clone.find(".remove-repeater-button").on("click", removeRepeater); // Set save trigger on ajax forms
    ...
    $(".repeater-list__add-repeater-trigger").on("click", function (e) {
      ...
      e.preventDefault(); // Add Profile Elements

      addRepeater(this);
    });
    $(".remove-repeater-button").on("click", removeRepeater); // Profile List Handlers ===============================================
    // Add Profile Element

    function addProfileElement(trigger) {
      var clone = cloneRepeatingElement(trigger, ".profile-list", ".profile-element-list", ".profile-null", ".profile-element.template", true); // Reactivate Nested Relatives

      loadProfileRelatives(); // Set save trigger on ajax forms
    ...
    function loadProfileRelatives() {
      ...
      $(".profile-relative__add-trigger").on("click", function (e) {
        // Prevent Default Functions
        e.preventDefault(); // Add Profile Relative
    ...
    loadProfileRelatives(); // Remove Profile Relative
    ...
    function loadProfileRelativeDeletion() {
      ...
      $(".profile-relative__remove-trigger").on("click", function (e) {
        // Prevent Default Functions
        e.preventDefault(); // Delete Profile Relative
    ...
    loadProfileRelativeDeletion(); // Experience Handlers =================================================
    // Degrees

    function addDegree(trigger) {
      // Get Wrapper
      var wrapper = $(".application-post__experience-wrapper"); // Get Template

      var template = $(".application-post__accordion--degree.template").clone(); // Remove Template Class

      template.removeClass("template"); //Set ids and form names to be unique

      individualizeFormIdsAndNames(template, wrapper); // Append Clone to the Wrapper
    ...
    function addCourse(trigger) {
      ...
      var template = $(".application-post__accordion--course.template").clone(); // Remove Template Class
    ...
    function addWork(trigger) {
      ...
      var template = $(".application-post__accordion--work.template").clone(); // Remove Template Class
    ...
    function addTask(trigger) {
      // Get Wrapper
      var wrapper = $(".manager-jobs__create-task-wrapper"); // Get Template

      var template = $(".manager-jobs__create-task.template").clone(); // Get New ID
      ...
      template.removeClass("template"); //TODO: replace with call to individualizeFormIdsAndNames(template, wrapper);
      //TODO: This requires changes to JobController@create, because the id would change places
      // Assign the New ID

      template.attr("data-task-id", newID); // Add newID as suffix to all "id" and "for" attributes
    ...
    deleteTaskTrigger(); // Skills

    function addSkill(trigger) {
      // Get Parent
      var parent = $(trigger).parents(".manager-jobs__skill-wrapper"); // Get Wrapper

      var wrapper = parent.find(".manager-jobs__create-skill-wrapper"); // Get Template

      var template = parent.find(".manager-jobs__create-skill.template").clone(); // Remove Template Class
    ...
    deleteSkillTrigger(); // Questions

    function addQuestion(trigger) {
      // Get Wrapper
      var wrapper = $(".manager-jobs__create-question-wrapper"); // Get Template

      var template = $(".manager-jobs__create-question.template").clone(); // Get New ID
      ...
      template.attr("data-question-id", newID); // Add newID as suffix to all "id" and "for" attributes
    ...
    deleteQuestionTrigger(); // Screening Plan Copy Function =====================================================
